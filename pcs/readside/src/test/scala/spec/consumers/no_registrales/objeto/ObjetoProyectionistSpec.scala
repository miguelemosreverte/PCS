...
trait ObjetoProyectionistSpec extends ProyectionistSpec[ObjetoEvents, ObjetoMessageRoots] {
  implicit val tableName: TableName = TableName("read_side.buc_sujeto_objeto")

  def validateObjetoReadside: Any => Any = {
    case snapshot: ObjetoEvents.ObjetoSnapshotPersisted =>
      val mappedEvent: Map[String, String] =
      ProjectionTestkit read ObjetoMessageRoots(snapshot.sujetoId, snapshot.objetoId, snapshot.tipoObjeto)
  ...
  "change read_side.buc_sujeto_objeto" in {
    ...
    validateObjetoReadside(snapshot)
  }

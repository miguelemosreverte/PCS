...
trait ObligacionProyectionistSpec extends ProyectionistSpec[ObligacionEvents, ObligacionMessageRoots] {
  implicit val tableName: TableName = TableName("read_side.buc_obligaciones")

  def validateObligacionReadside: Any => Any = {
  ...
  s"updated the row from readside.buc_obligaciones" in {
    ...
    validateObligacionReadside(snapshot)
  }

  "ObligacionRemoved" should
  s"removed the row from readside.buc_obligaciones" in {
    ...
    ProjectionTestkit process eventEnvelope(snapshot)
    validateObligacionReadside(snapshot)
    ProjectionTestkit process eventEnvelope(remove)

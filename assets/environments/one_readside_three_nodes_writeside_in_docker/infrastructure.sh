...
docker-compose -f assets/docker-compose/docker-compose-kafka.yml  exec kafka kafka-topics --create --bootstrap-server localhost:9092 --replication-factor 1 --partitions 120 --topic DGR-COP-ACTIVIDADES
docker-compose -f assets/docker-compose/docker-compose-kafka.yml  exec kafka kafka-topics --create --bootstrap-server localhost:9092 --replication-factor 1 --partitions 120 --topic DGR-COP-SUJETO-TRI
docker-compose -f assets/docker-compose/docker-compose-kafka.yml  exec kafka kafka-topics --create --bootstrap-server localhost:9092 --replication-factor 1 --partitions 120 --topic DGR-COP-OBLIGACIONES-TRI
